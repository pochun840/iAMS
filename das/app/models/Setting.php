...
class Setting{
    ...
    {
        $sql = "UPDATE `device` SET device_id =?, device_name=?, device_torque_unit=?, device_language=?, device_batch_mode=?, device_buzzer_mode=?,device_memory=?, device_diskfullwarning=?, device_torque_filter=?";
        $statement = $this->db->prepare($sql);
        $results = $statement->execute([ $data['control_id'],$data['control_name'],$data['torque_unit'],$data['select_language'],$data['batch_mode_option'],$data['buzzer_mode_option'],$data['blackout_recovery_option'],$data['Diskfull_Warning'],$data['Torque_Filter'] ] );
